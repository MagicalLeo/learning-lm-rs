...
fn mlp(
    ...
    OP::swiglu(gate, up);
    OP::matmul_transb(residual, 0., gate, w_down, 1.);
    let residual_data = unsafe {residual.data_mut()};
    let hidden_states_data = hidden_states.data();
    assert_eq!(residual_data.len(), hidden_states_data.len(), "Tensor size mismatch");
